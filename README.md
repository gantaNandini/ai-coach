...
RAG Pipeline
Text → chunks (512 tokens, 64 overlap) via langchain-text-splitters
Chunks → embeddings via BAAI/bge-small-en-v1.5 (384-dim)
HNSW vector search via pgvector (falls back to full-text search if pgvector unavailable)
Cross-encoder reranking via BAAI/bge-reranker-base (runs in thread executor — non-blocking)
Top chunks injected as {{knowledge}} in coaching prompt
Citations attached to feedback report with relevance scores
...
Contributing
Branch from main
Follow the rules in .kiro/steering/tech-stack.md
All 29 live tests must pass before opening a PR: powershell -ExecutionPolicy Bypass -File test_platform.ps1
No fake/stub data in production components — use skeletons/loading states
